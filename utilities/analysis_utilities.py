...
def run_mmseqs_clustering(input_fasta: str, output_prefix: str, min_identity: float = 0.4,
                          coverage: float = 0.8, sensitivity: float = 7.5):
    """
...
def split_data_by_clusters(cluster_mapping: Dict[str, List[str]], train_ratio: float = 0.8,
    ...
    print(f"Split complete: {len(train_indices)} sequences in Train, {len(validation_indices)} in Validation.")
    print(f"Number of clusters: {len(representatives)}"
          f" (Train: {len(train_representatives)}, Validation: {len(validation_representatives)})")
