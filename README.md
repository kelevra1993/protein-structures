...
├── data_examples                      # Data for testing and debugging
│   ├── data_splits                    # Sample Train/Validation/Test JSON split files
│   └── openfold                       # Truncated MSAs and structures
...
├── embedders                          # Initial embedding layers
│   ├── input_embedder.py              # MSA and Pair initial embeddings
│   ├── extra_msa_embedder.py          # Extra MSA sequence embeddings and ExtraMsaStack
│   └── recycling_embedder.py          # Structural recycling embeddings
...
│   │   ├── msa.py                     # Functions for parsing and encoding MSAs
│   │   └── structure.py               # Parsing atomic coordinates and building structural labels
│   ├── debugger                       # Visualization tools for debugging 3D frames
│   ├── constants.py                   # Amino acid residues and dictionaries
...
The project relies on YAML configuration files to dictate data paths, training loop hyperparameters, and the precise architectural dimensions of every AlphaFold II module.

- **`mps_configuration.yaml`**: Optimized for testing smaller model variants on local hardware (e.g., M2 MacBook Pro).
- **`cuda_configuration.yaml`**: Production-ready settings used for training on an **NVIDIA DGX Spark**.
- **`template_configuration.yaml`**: A clean baseline for creating custom experiments.

**Hardware-Specific Loading**: The `main.py` entry point automatically selects the configuration file matching your detected hardware (e.g., `cuda_configuration.yaml` if an NVIDIA GPU is found). To use a custom configuration, you can modify the path in `main.py`.
...
- **Experiment & Data Configurations**: Manages global training loop settings, data paths, and how the data loader formats, crops, and samples sequences.
- **GlobalConfiguration**: Defines universal embedding dimensions used across all sub-modules to maintain architectural consistency.
- **Embedders**: Project raw sequence features, deep MSA features, and structural recycling data into initial continuous representations.
- **ExtraMsaStack**: Processes deep, unclustered MSA sequences to enrich the pair representation with co-evolutionary signals.
- **EvoformerStack**: The core engine that iteratively refines both the MSA and pairwise structural representations.
- **StructureModule**: Uses Invariant Point Attention (IPA) to generate explicit 3D atomic coordinates.
- **LddtModule**: Predicts the per-residue confidence score (pLDDT) of the generated structure.
