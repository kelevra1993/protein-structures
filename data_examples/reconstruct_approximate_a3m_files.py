            ...
            for seq_meta in sequences:
                ...
                for t in seq_tokens:
                    ...
                    if token_id < total_tokens:
                        ...
                        # todo a print for when we have something that we do not know about
                        print(f"This Token ID does not meet our requirements {token_id=}>={total_tokens=}")
...
if __name__ == "__main__":
    ...
    for file_path in tqdm(glob.glob(str(open_fold_folder / "*.npz"))):
    ...
    for file_path in tqdm(glob.glob(str(rcsb_folder / "*.npz"))):
        input_file = file_path
        output_file = convert_extension(file_path=file_path, before="npz", after="a3m")
        reconstruct_boltz_a3m(input_file, output_file, filler_char="x")
